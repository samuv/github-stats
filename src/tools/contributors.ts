...
import { LimitArgs, GitHubContributor } from '../types.js';
...
export const createContributorsHandlers = (github: GitHubService, truncateResponse: any) => ({
  get_contributors: async (args: LimitArgs, maxChars?: number) => {
    const result = await github.getContributors(args.repository, args.limit || 30);
    
    // Include all contributors but with only essential fields
    const condensedContributors = result.map((contributor: GitHubContributor) => ({
    ...
    const { data: finalData } = truncateResponse(responseData, 'contributors', maxChars);
