...
import { RepositoryArgs, LimitArgs } from '../types.js';
...
export const stargazerTools: Tool[] = [
  {
    ...
    description: 'Get comprehensive star history analytics inspired by star-history.com with growth trends and milestones',
    inputSchema: {
      type: 'object',
      properties: {
        repository: {
  ...
  },
  {
    ...
    description: 'Get simplified star history metrics with star-history.com URL for visualization',
    inputSchema: {
      type: 'object',
      properties: {
        repository: {
  ...
  },
  {
    ...
    description: 'Get complete star history analytics using ALL stargazers data (more accurate than limited sampling)',
    inputSchema: {
      type: 'object',
      properties: {
        repository: {
  ...
  {
    name: 'get_stargazers',
    description: 'Get stargazers for a repository with customizable limit (efficient for smaller datasets)',
    inputSchema: {
      type: 'object',
      properties: {
        repository: {
        ...
        limit: {
          type: 'number',
          description: 'Maximum number of stargazers to return (default: 100, max: 10000)',
          default: 100
        }
      },
      required: ['repository']
  ...
  },
  {
    ...
    description: 'Get ALL stargazers for a repository (unlimited pagination through all 100-item pages)',
    inputSchema: {
      type: 'object',
      properties: {
        repository: {
  ...
  },
  {
    ...
    description: 'Analyze influential developers who have starred the repository based on follower count and activity. Adaptively analyzes ALL stargazers when rate limits allow, or uses smart sampling when limits are tight.',
    inputSchema: {
      type: 'object',
      properties: {
        repository: {
        ...
        limit: {
          type: 'number',
          description: 'Number of top influencers to return (default: 100, max: 200). Analysis scope adapts to GitHub API rate limits automatically.',
          default: 100
        }
      },
      required: ['repository']
...
export const createStargazerHandlers = (github: GitHubService, truncateResponse: any) => ({
  ...
  get_stargazers: async (args: LimitArgs, maxChars?: number) => {
    ...
    const condensedStargazers = result.map((stargazer: any) => ({
    ...
    const { data: finalData } = truncateResponse(responseData, 'stargazers', maxChars);
