...
import { RepositoryArgs, GitHubRepository } from '../types.js';
...
export const repositoryTools: Tool[] = [
  {
    ...
    description: 'Get basic information about a GitHub repository including stars, forks, description, etc.',
    inputSchema: {
      type: 'object',
      properties: {
        repository: {
  ...
  },
  {
    ...
    description: 'Get comprehensive statistics for a repository including all available data',
    inputSchema: {
      type: 'object',
      properties: {
        repository: {
...
export const createRepositoryHandlers = (github: GitHubService, truncateResponse: any) => ({
  ...
  get_language_stats: async (args: RepositoryArgs) => {
    ...
    const totalBytes = result.reduce((sum: number, lang: any) => sum + lang.bytes, 0);
  ...
  get_comprehensive_stats: async (args: RepositoryArgs, maxChars?: number) => {
    const result = await github.getComprehensiveStats(args.repository);
    
    // Add some calculated summaries
    const languageTotal = Object.values(result.languages).reduce((sum: number, bytes: any) => sum + bytes, 0);
    const languageBreakdown = Object.entries(result.languages).map(([language, bytes]: [string, any]) => ({
      language,
      percentage: ((bytes / languageTotal) * 100).toFixed(2)
    })).sort((a, b) => parseFloat(b.percentage) - parseFloat(a.percentage));

    // Safely handle commitActivity which might be null/undefined
    const commitActivityArray = Array.isArray(result.commitActivity) ? result.commitActivity : [];
    const totalCommits = commitActivityArray.reduce((sum: number, week: any) => sum + (week?.total || 0), 0);
    ...
    const condensedResult = {
      ...
      issues_summary: {
        ...
        recent_3_issues: result.openIssues.slice(0, 3).map(i => ({
          number: i.number,
          title: i.title.substring(0, 100) + (i.title.length > 100 ? '...' : ''),
      ...
      pull_requests_summary: {
        ...
        recent_3_prs: result.openPullRequests.slice(0, 3).map(pr => ({
          number: pr.number,
          title: pr.title.substring(0, 100) + (pr.title.length > 100 ? '...' : ''),
      ...
      release_analytics_summary: {
        ...
        average_downloads_per_release: result.releaseAnalytics.average_downloads_per_release,
        most_downloaded_release: result.releaseAnalytics.most_downloaded_release,
      ...
      summary: {
        ...
        repository_age_days: Math.floor((Date.now() - new Date(result.repository.created_at).getTime()) / (1000 * 60 * 60 * 24)),
