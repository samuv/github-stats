...
export const getAllStargazersWithTimestamps = (client: Octokit) => 
  withErrorHandling(async (identifier: string): Promise<Array<{ login: string; starred_at: string | null }>> => {
    ...
    while (true) {
      try {
        ...
        if (response.data.length === 0) {
          console.error(`Finished fetching stargazers. Total: ${stargazers.length}`);
          break;
        }
        ...
        console.error(`Fetched page ${page}: ${batchStargazers.length} stargazers (total: ${stargazers.length})`);
        ...
        if (page % 10 === 0) {
          console.error(`Rate limiting: Pausing for 1 second after ${page} pages...`);
          await new Promise(resolve => setTimeout(resolve, 1000));
        }
      } catch (error: any) {
        console.error('Star timestamp API error, using fallback:', error.message);
...
export const getStargazersWithTimestamps = (client: Octokit) => 
  withErrorHandling(async (identifier: string, limit = 1000): Promise<Array<{ login: string; starred_at: string | null }>> => {
    const { owner, repo } = parseRepoIdentifier(identifier);
    const stargazers: Array<{ login: string; starred_at: string | null }> = [];
    let page = 1;
    ...
    while (stargazers.length < limit) {
      try {
        ...
        return getStargazersFallback(client)(identifier, limit);
      }
    }
...
const getAllStargazersFallback = (client: Octokit) => 
  async (identifier: string): Promise<Array<{ login: string; starred_at: string | null }>> => {
    const { owner, repo } = parseRepoIdentifier(identifier);
    const stargazers: Array<{ login: string; starred_at: string | null }> = [];
    let page = 1;
    const perPage = 100; // Maximum allowed by GitHub API
    ...
    while (true) {
      ...
      if (response.data.length === 0) {
        console.error(`Fallback finished. Total stargazers: ${stargazers.length}`);
        break;
      }
      ...
      stargazers.push(...batchStargazers);
      console.error(`Fallback page ${page}: ${batchStargazers.length} stargazers (total: ${stargazers.length})`);
      ...
      if (page % 10 === 0) {
        console.error(`Fallback rate limiting: Pausing for 1 second after ${page} pages...`);
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
    }
...
const getStargazersFallback = (client: Octokit) => 
  async (identifier: string, limit: number): Promise<Array<{ login: string; starred_at: string | null }>> => {
    const { owner, repo } = parseRepoIdentifier(identifier);
    const stargazers: Array<{ login: string; starred_at: string | null }> = [];
    let page = 1;
    const perPage = Math.min(100, limit);
...
export const calculateStarHistory = (
  ...
  const starsByDate = groupBy(
    ...
    stargazer => stargazer.starred_at!.split('T')[0]
  );
...
export const getInfluencerProfiles = (client: Octokit) => 
  async (
    stargazers: Array<{ login: string; starred_at: string | null }>,
    batchSize = 10,
    delayMs = 1000
  ): Promise<InfluencerDeveloper[]> => {
    const fetchUserProfile = async (stargazer: { login: string; starred_at: string | null }) => {
      try {
        ...
        const accountAge = Math.floor(calculateAgeInDays(user.created_at) / 365);
        const influenceScore = calculateInfluenceScore(user.followers, user.public_repos, accountAge);
...
export const analyzeInfluencers = (influencers: InfluencerDeveloper[]): InfluencerAnalytics => {
  const sortedInfluencers = [...influencers].sort((a, b) => b.influence_score - a.influence_score);
...
// Helper functions
const calculateInfluenceScore = (followers: number, repos: number, accountAgeYears: number): number => {
...
const findBestGrowthDay = (historyPoints: StarHistoryPoint[]) => {
  const bestDay = historyPoints
    ...
    .reduce((max, point) => point.change > max.change ? point : max, { change: 0, date: '', stars: 0 });
  
  return bestDay.change > 0 ? { date: bestDay.date, stars_gained: bestDay.change } : null;
};

const findWorstGrowthDay = (historyPoints: StarHistoryPoint[]) => {
  const worstDay = historyPoints
    .filter(point => point.change < 0)
    .reduce((min, point) => point.change < min.change ? point : min, { change: 0, date: '', stars: 0 });
  
  return worstDay.change < 0 ? { date: worstDay.date, stars_lost: Math.abs(worstDay.change) } : null;
};
...
const calculateMilestones = (historyPoints: StarHistoryPoint[], currentStars: number) => {
...
const calculateInfluenceDistribution = (influencers: InfluencerDeveloper[]) => ({
...
const calculateGeographicDistribution = (influencers: InfluencerDeveloper[]): Record<string, number> => {
...
const calculateCompanyDistribution = (influencers: InfluencerDeveloper[]): Record<string, number> => {
  const distribution: Record<string, number> = {};
  influencers.forEach(user => {
    if (user.company) {
...
const calculateInfluenceMetrics = (influencers: InfluencerDeveloper[], totalFollowers: number) => {
...
const identifyNotableStargazers = (influencers: InfluencerDeveloper[]) => {
  ...
  influencers
    .filter(u => u.company && notableCompanies.some(company => 
  ...
  const topInfluencers = [...influencers].sort((a, b) => b.influence_score - a.influence_score);
  topInfluencers
    .slice(0, 5)
    .forEach(user => {
...
export const calculateOptimalAnalysisLimit = async (client: Octokit): Promise<{
  ...
  try {
    ...
    const now = new Date();
    const minutesUntilReset = Math.max(0, (resetTime.getTime() - now.getTime()) / (1000 * 60));
    
    console.error(`Rate limit status: ${remainingRequests}/${core.limit} remaining, resets in ${Math.round(minutesUntilReset)} minutes`);
    ...
    console.error(`Failed to check rate limits, using safe defaults: ${error.message}`);
