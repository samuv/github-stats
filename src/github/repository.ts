...
export const getLanguages = (client: Octokit) =>
	withErrorHandling(
		async (identifier: string): Promise<GitHubLanguageStats> => {
			const { owner, repo } = parseRepoIdentifier(identifier);
...
export const getContributors = (client: Octokit) =>
	withErrorHandling(
		async (identifier: string, limit = 30): Promise<GitHubContributor[]> => {
			const { owner, repo } = parseRepoIdentifier(identifier);
...
export const getCommitActivity = (client: Octokit) =>
	withErrorHandling(
		async (identifier: string): Promise<GitHubCommitActivity[]> => {
			const { owner, repo } = parseRepoIdentifier(identifier);

			// Use retry logic to handle GitHub's statistics computation delay
			const data = await withRetry(
				async (): Promise<GitHubCommitActivityResponse> => {
					const response = await client.rest.repos.getCommitActivityStats({
						owner,
						repo,
					});
					return response.data;
...
export const getOpenIssues = (client: Octokit) =>
	withErrorHandling(
		async (identifier: string, limit = 30): Promise<GitHubIssue[]> => {
			const { owner, repo } = parseRepoIdentifier(identifier);
...
export const getOpenPullRequests = (client: Octokit) =>
	withErrorHandling(
		async (identifier: string, limit = 30): Promise<GitHubPullRequest[]> => {
			const { owner, repo } = parseRepoIdentifier(identifier);
...
export const searchRepositories = (client: Octokit) =>
	withErrorHandling(
		async (query: string, limit = 10): Promise<GitHubSearchRepository[]> => {
...
export const getTrafficReferrers = (client: Octokit) =>
	withErrorHandling(
		async (identifier: string): Promise<GitHubTrafficReferrer[]> => {
			const { owner, repo } = parseRepoIdentifier(identifier);
			...
			try {
				...
				if (
					...
					console.warn(
						`Insufficient permissions to access traffic data for ${identifier}. Repository owner/admin access required.`,
					);
					return [];
				}
				if (
