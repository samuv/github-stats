...
import type { GitHubRelease, ReleaseAnalytics, DownloadStats } from '../types.js';
import { parseRepoIdentifier, withErrorHandling, sum, sortByField, calculateDaysBetween } from '../utils/helpers.js';

// Release fetching operations
export const getReleases = (client: Octokit) => 
  withErrorHandling(async (identifier: string, limit = 10): Promise<GitHubRelease[]> => {
    ...
    try {
      ...
      if (error.status === 404) {
        console.warn(`No releases found for ${identifier} or repository not accessible`);
        return [];
      }
      throw error;
    }
...
export const getLatestRelease = (client: Octokit) => 
  withErrorHandling(async (identifier: string): Promise<GitHubRelease | null> => {
    const { owner, repo } = parseRepoIdentifier(identifier);
    try {
      const response = await client.rest.repos.getLatestRelease({ owner, repo });
      return response.data;
    } catch (error: any) {
      if (error.status === 404) {
        return null;
...
export const calculateReleaseMetrics = (releases: GitHubRelease[]): ReleaseAnalytics => {
  ...
  const downloadTrends = releases.map(release => ({
    ...
    asset_breakdown: release.assets.map(asset => ({
      name: asset.name,
  ...
  const totalDownloads = sum(downloadTrends.map(trend => trend.total_downloads));
  const totalAssets = sum(releases.map(release => release.assets.length));
  const averageDownloads = releases.length > 0 ? Math.round(totalDownloads / releases.length) : 0;

  const sortedDownloadTrends = [...downloadTrends].sort((a, b) => b.total_downloads - a.total_downloads);
...
export const calculateDownloadStats = (releases: GitHubRelease[]): DownloadStats => {
  if (!Array.isArray(releases) || releases.length === 0) {
    return {
      total_downloads: 0,
  ...
  const sortedByDownloads = [...downloadsByRelease].sort((a, b) => b.total_downloads - a.total_downloads);
...
const calculateAssetTypes = (releases: GitHubRelease[]) => {
  const assetTypes: Record<string, { count: number; total_downloads: number; total_size_mb: number }> = {};
  ...
  releases.forEach(release => {
    release.assets.forEach(asset => {
      ...
      if (!assetTypes[extension]) {
        assetTypes[extension] = { count: 0, total_downloads: 0, total_size_mb: 0 };
      }
...
const calculatePrereleaseStats = (releases: GitHubRelease[]) => {
  ...
  return {
    ...
    percentage_prereleases: releases.length > 0 
      ? Math.round((prereleases.length / releases.length) * 10000) / 100 
      : 0
  };
};

const calculateDownloadDistribution = (assets: Array<{ name: string; downloads: number; size_mb: number }>) => {
