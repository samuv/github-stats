...
export const createGitHubService = (config: GitHubServiceConfig = {}) => {
  ...
  const releaseOps = {
    ...
    getReleaseAnalytics: async (identifier: string): Promise<ReleaseAnalytics> => {
  ...
  // Star operations
  const starOps = {
    getStarHistory: async (identifier: string): Promise<StarHistoryAnalytics> => {
    ...
    getCompleteStarHistory: async (identifier: string): Promise<StarHistoryAnalytics> => {
      console.error(`Fetching COMPLETE star history for ${identifier}...`);
      const [repository, stargazers] = await Promise.all([
        repositoryOps.getRepository(identifier),
        getAllStargazersWithTimestamps(client)(identifier)
      ]);
      console.error(`Analyzing ${stargazers.length} total stargazers for complete history...`);
      return calculateStarHistory(repository, stargazers);
    },
    ...
    getInfluencerAnalytics: async (identifier: string, returnLimit = 100): Promise<InfluencerAnalytics> => {
      console.error(`Analyzing influencer stargazers for ${identifier}...`);
      
      // Check rate limit status and adjust analysis accordingly
      const { analysisLimit, shouldUseAllStargazers } = await calculateOptimalAnalysisLimit(client);
      console.error(`Rate limit check: Will analyze up to ${analysisLimit} stargazers (token: ${process.env.GITHUB_TOKEN ? 'yes' : 'no'})`);
      
      let stargazers;
      if (shouldUseAllStargazers) {
        // Get ALL stargazers when we have good rate limits
        const allStargazers = await getAllStargazersWithTimestamps(client)(identifier);
        ...
        const stargazersToAnalyze = allStargazers.length > analysisLimit 
        ...
        stargazers = await getStargazersWithTimestamps(client)(identifier, analysisLimit);
      }
      
      console.error(`Analyzing detailed profiles for ${stargazers.length} stargazers...`);
      
      const influencers = await getInfluencerProfiles(client)(stargazers, batchSize, delayMs);
      ...
      if (result.top_influencers && result.top_influencers.length > returnLimit) {
