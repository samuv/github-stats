...
export const sortByField = <T>(field: keyof T, desc = false) => 
  (array: T[]): T[] => {
...
// String operations
export const parseRepoIdentifier = (identifier: string): { owner: string; repo: string } => {
...
const hasArrayField = (data: HandlerResponseData, fieldName: string): data is HandlerResponseData & Record<string, any[]> => {
  return typeof data === 'object' && data !== null && fieldName in data && Array.isArray((data as any)[fieldName]);
};

// Dynamic token limit - can be overridden per request
export const truncateResponse = (data: HandlerResponseData, arrayFieldName: string, maxChars?: number): { data: HandlerResponseData; wasTruncated: boolean } => {
  ...
  const originalArray = data[arrayFieldName];
  ...
  while (targetSize > 0) {
    truncatedData = {
      ...
      truncation_info: {
        ...
        showing: targetSize,
        truncated: true,
        reason: "Response exceeded token limit - use smaller limit parameter or pagination"
      }
    } as HandlerResponseData;
...
export function validateToolArgs(toolName: string, args: any, tools: any[]): boolean {
...
export function isValidToolName<T extends Record<string, any>>(
  name: string, 
  handlers: T
