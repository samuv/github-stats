...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
	...
	const tokenLimit = (args.max_tokens ||
		args.token_limit ||
		args.max_response_tokens) as number | undefined;
	const maxChars = tokenLimit ? tokenLimit * 4 : undefined; // Rough conversion

	if (tokenLimit) {
		console.error(
			`Token limit hint provided: ${tokenLimit} tokens (~${maxChars} chars)`,
		);
	}
	...
	try {
		...
		return await handler(args as never, maxChars);
	} catch (error: unknown) {
...
async function main() {
	...
	if (!process.env.GITHUB_TOKEN) {
		console.error(
			"Warning: No GITHUB_TOKEN environment variable set. API rate limits will be lower.",
		);
		console.error(
