...
export class GitHubClient {
	...
	async getLanguageStats(repoIdentifier: string): Promise<GitHubLanguageStats> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to fetch language stats: ${message}`);
		}
	}
	...
	async getContributors(
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to fetch contributors: ${message}`);
		}
	}
	...
	async getCommitActivity(
		...
		try {
			...
			const data = await withRetry(
				...
				(
					data: GitHubCommitActivityResponse,
				): data is GitHubCommitActivity[] => {
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to fetch commit activity: ${message}`);
		}
	}
	...
	async getOpenIssues(
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to fetch open issues: ${message}`);
		}
	}
	...
	async getOpenPullRequests(
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to fetch open pull requests: ${message}`);
		}
	}
	...
	async getComprehensiveStats(repoIdentifier: string): Promise<GitHubStats> {
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to fetch comprehensive stats: ${message}`);
		}
	}
	...
	async getAllReleases(repoIdentifier: string): Promise<GitHubRelease[]> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to fetch all releases: ${message}`);
		}
	}
		...
		try {
			...
			releases.forEach((release) => {
				...
				release.assets.forEach((asset) => {
					...
					// Track asset types
					const extension =
						asset.name.split(".").pop()?.toLowerCase() || "unknown";
				...
				downloadTrends.push({
					...
					asset_breakdown: assetBreakdown.sort(
			...
			const releasesByDownloads = downloadTrends.sort(
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to get release analytics: ${message}`);
		}
	}
	...
	async getDownloadStats(repoIdentifier: string): Promise<DownloadStats> {
		try {
			...
			const downloadsByRelease = releases
				.map((release) => {
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to get download stats: ${message}`);
		}
	}
	...
	async getLatestRelease(
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to fetch latest release: ${message}`);
		}
	}
	...
	async getStarHistory(repoIdentifier: string): Promise<StarHistoryAnalytics> {
		...
		try {
			...
			historyPoints.forEach((point) => {
				if (
					point.change > 0 &&
					(!bestGrowthDay || point.change > bestGrowthDay.stars_gained)
				) {
					bestGrowthDay = { date: point.date, stars_gained: point.change };
				}
				if (
					point.change < 0 &&
					(!worstGrowthDay || point.change < worstGrowthDay.stars_lost)
				) {
					worstGrowthDay = {
						date: point.date,
						stars_lost: Math.abs(point.change),
					};
				}
			});
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to get star history: ${message}`);
		}
	}
	 ...
	 */
	private async getStargazersWithTimestamps(
		owner: string,
		repo: string,
	): Promise<Array<{ starred_at: string | null }>> {
		try {
			...
			while (page <= maxPages) {
				...
				stargazers.push(
					...response.data.map((item: { starred_at?: string }) => ({
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error("Star timestamp API not available:", message);
			return [];
		}
	}
	...
	async getSimpleStarHistory(repoIdentifier: string): Promise<{
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to get simple star history: ${message}`);
		}
	}
	...
	async getInfluencerStargazers(
		...
		try {
			...
			const stargazersData = await this.getDetailedStargazers(
				owner,
				repo,
				limit,
			);
			...
			console.error(
				`Found ${stargazersData.length} stargazers, fetching detailed profiles...`,
			);
			...
			for (let i = 0; i < stargazersData.length; i += batchSize) {
				...
				const batchPromises = batch.map(async (stargazer) => {
					try {
						...
						console.error(
							`Failed to fetch user ${stargazer.login}:`,
			...
			const influenceDistribution = {
				mega_influencers: influencers.filter((u) => u.followers >= 10000)
					.length,
			...
			influencers.forEach((user) => {
				if (user.location) {
					...
					geographicDistribution[location] =
						(geographicDistribution[location] || 0) + 1;
				}
			});
			...
			return {
				...
				metrics: {
					...
					influence_concentration:
						Math.round(influenceConcentration * 100) / 100,
				},
				notable_stargazers: notableStargazers,
			};
		} catch (error: unknown) {
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Failed to get influencer stargazers: ${message}`);
		}
	}

	/**
	 * Type guard to check if item has user property (timestamped stargazer API)
	 */
	private isStargazerWithUser(
		item: unknown,
	...
	/**
	 * Type guard to check if item is a regular user object (non-timestamped API)
	 */
	private isRegularUser(item: unknown): item is { login: string; id: number } {
		return Boolean(
			item &&
				typeof item === "object" &&
				item !== null &&
	 ...
	 */
	private async getDetailedStargazers(
		owner: string,
		repo: string,
		limit: number,
		...
		try {
			...
			while (stargazers.length < limit) {
				...
				const batchStargazers = response.data
					.map((item) => {
						if (this.isStargazerWithUser(item)) {
							return {
								login: item.user?.login || "",
								starred_at: item.starred_at || null,
							};
						}
						// Fallback for regular stargazer API response
						if (this.isRegularUser(item)) {
							return {
								login: item.login || "",
								starred_at: null,
							};
			...
			console.error(
				"Star timestamp API not available, using fallback:",
				error instanceof Error ? error.message : String(error),
			);
	 ...
	 */
	private async getFallbackStargazers(
		owner: string,
		repo: string,
		limit: number,
	): Promise<Array<{ login: string; starred_at: string | null }>> {
